...
- Otherwise a Lucide line icon that says what it is: *Subnetting* → network.svg, *SQL* → database.svg, *NVMe* → hard-drive.svg, and so on. The Rust-server plugins (Oxide / uMod, CopyPaste, Raidable Bases) and Shockbyte have no official marks, so they use icons too.
